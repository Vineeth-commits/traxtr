...
def test_enter_choice():
    while True:
        ...
        if(choice==1):
            ...
            alert_price = str(test_input_decimal())
            current_time = datetime.datetime.now()
            id = test_get_id_by_username(verify_user)
            webcode = "snapdeal"
            test_database_insert_data_chart(id,title,price,product_url,current_time,webcode,alert_price)
        if(choice==2):
            product_url = input("Enter the url")
            headers = {"user agent":user_agent_list}
            try:
...
def test_database_insert_data_chart(id,title,price,product_url,timestamp,webcode,alert_price):
